<?php   
    ...
    class Forms {
        ...
        function __construct($row = null,$section = null,$parent_id = null,$link=null) {
            $this->row = $row;
        ...
        */
        function create($options = null){
            ...
            if(is_array($options)){
                ...
                if(isset($options["class"])) $return .= "class='".$options['class']."' form"; else $return .= "class='form-horizontal form' ";                
        ...
        */
        function  input($name,$label = null,$attributes = null){
            if(isset($this->row)){
                    ...
                    if(isset($attributes["type"])){
                        if(($attributes["type"] == "radio") || ($attributes["type"] == "checkbox")) {
                            if(isset($attributes["value"])){
                                if($this->row[$name] == $attributes["value"]) $attributes["checked"] = 1;
                            }
                        } else {
                            if(!isset($attributes["value"])) $attributes["value"] = utf8_encode ($this->row[$name]);
                        } 
                    } else {
                        if(!isset($attributes["value"])) $attributes["value"] = utf8_encode ($this->row[$name]);                        
                    } 
               ...
               if((isset($attributes["type"])) && (@$attributes["type"] == "radio") || (@$attributes["type"] == "checkbox")) {                    
                    if(isset($attributes["checked"])) $attributes["checked"] = 1;                    
                } 
            }
            ...
            //Creating the element block
                if(isset ($attributes['type']) && $attributes['type'] == 'file'){
                    ...
                    if(isset ($attributes['value'])){
                        ...
                        $input = "<input id='".$name."' type='hidden' name='".$name."' value='' />";
                    }
                }else{
                ...
                if((isset($attributes["type"]) && ($attributes["type"] != "radio") && ($attributes["type"] != "checkbox")) || (!isset($attributes["type"]))) {
                ...
                if(is_array($attributes)){
                    ...
                    if(isset($attributes["checked"]) && $attributes["checked"] == 1) $input .= "checked='checked' ";
                    ...
                    if(isset($attributes["disabled"])) $input .= "disabled ";                    
                    if(isset($attributes["inherit"])) $input .= "value='".stripcslashes (str_replace ("'", "&apos;", $this->row[$attributes["inherit"]]))."'";
        ...
        function select($name,$label = null,$options = null,$selected_value = null,$db = null,$attributes = null) {
            ...
            $input = $this->element_block($name);
                ...
                if(is_array($attributes)){
                    ...
                    if(isset($attributes["required"])) $input .= "required ";
                    if(isset($attributes["disabled"])) $input .= "disabled ";
                    if(isset($attributes["multiple"])) $input .= "multiple ";
                    if(isset($attributes["size"])) $input .= "size = '".$attributes["size"]."' ";
            ...
            if(is_array($options)){
                ...
                if(isset($db["link"]) && isset($db["value_field"]) && isset($db["show_field"]) && isset($db["table"])){
                    if((isset($db["parent_id"]) && isset($this->row[$db["parent_id"]])) || (!isset($db["parent_id"]))){
                        $sql = "SELECT ".$db["value_field"].",".$db["show_field"]." FROM ".$db["table"];
                        if(isset($db["where"])) $sql .= " WHERE ".$db["where"];
                        if(isset($db["parent_id"]) && isset($db["where"])) $sql .= ",".$db["parent_id"]."=".$this->row[$db["parent_id"]];
                        if(isset($db["parent_id"]) && !isset($db["where"])) $sql .= " WHERE ".$db["parent_id"]."=".$this->row[$db["parent_id"]];
        ...
        function textarea($name,$label = null,$tiny = false,$attributes = null){
            $value = "";
            if(isset($this->row)){
                if(isset($this->row[$name])) $attributes["value"] = stripslashes(utf8_encode ($this->row[$name]));
            }
            ...
            if($tiny) $input .= "<a href='' onclick='media_show(); return false;' class='show-media btn-info btn'><i class='icon-picture icon-white'></i> Select image</a><div class='media-library' id='media-library'></div>";
            $input .= "<span class='help-inline'></span></div>".$this->element_block();
        ...
        */
        ...
        function end($complete = true){            
            if($complete) return $this->form .= "</form>";
            else return "</form>";
        }
    }
?>
