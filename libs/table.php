            ...
            if(count($rows)>0){
                ...
                for($i=0; $i<count($rows); $i++){
                    ...
                    $table .= "<tr id='row-".$rows[$i]["id"]."'>";
                        ...
                        //Num column
                        $table .= "<td id='cell-def-num-".($i+1)."'><a href='/happycms/".$this->url."/".$rows[$i][$this->url_key]."' class='cell-def-num-".($i+1)."'>".($num_row+1)."</a></td>";
                        ...
                        if(isset($options["image"])){             
                            ...
                            //Cover query
                            $sql = "SELECT title,id FROM images WHERE parent_id = ".$rows[$i][$cover_where_id]." AND section = '".$this->db_table."' ORDER BY orden LIMIT 1";
                            $image_url = "";
                            $cover_result = mysql_query($sql,$link);
                            if($cover_result){
                                $cover_row = mysql_fetch_assoc($cover_result);
                                //Cover image URL
                                $image_name = utf8_encode($cover_row["title"]);
                                $image_url = "img/cms/".$this->db_table."/".$rows[$i][$cover_where_id]."/thumbnails/thumb_small/".$image_name;                                                
                                
                            }
                            
                            //If doens't exist, show unknown image
                            if(!is_file($image_url)) $image_url = "img/cms/no-image.png";
                            //Cover column
                            ...
                            if(isset($options["description"][$header_keys[$j]])) {
                                ...
                                if(is_array($options["description"][$header_keys[$j]])){
                                    ...
                                    for($k=0; $k<count($options["description"][$header_keys[$j]]); $k++){                                        
                                        ...
                                        $table .= Text::truncate(str_replace(array("<p>","</p>","<br>"), "", utf8_encode($rows[$i][$options["description"][$header_keys[$j]][$k]])),150,array('ending' => '...','exact' => true));                                        
                        ...
                        if($this->db_table=="hats") $table .= "<a class='option-row' href='/happycms/hats/hat/stock/".$rows[$i]["id"]."'>Stock</a>";
                ...
                $this->table .= "<thead><tr><th>Registers</th></tr></thead><tbody><tr><td>There aren't any register.</td></tr></tbody>";
            }
        }
