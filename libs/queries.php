        ...
        */
        function save($post,$table,$validate,$id = null){
            ...
            if(is_array($post)){
                ...
                $validate_new_keys = array();
                for($i=0; $i<count($validate_keys); $i++){
                    if(!isset($validate[$validate_keys[$i]]["ignore"]) && array_search($validate_keys[$i], $post_keys) === false) array_push ($validate_new_keys, $validate_keys[$i]);
                }
                $post_keys = array_merge($validate_new_keys,$post_keys);                
                ...
                for($i=0; $i<count($post_keys)+1; $i++){
                    ...
                    if(!isset($validate[$post_keys[$i]]["ignore"])){
                        if((isset($post[$post_keys[$i]])) || (@$validate[$post_keys[$i]]["type"] == "checkbox")) {
                            $post_value = @trim(mysql_real_escape_string(utf8_decode($post[$post_keys[$i]])))."";    
                            $sql .= $post_keys[$i]." = ";
                            ...
                            if(isset($validate[$post_keys[$i]]["type"])) {
                                ...
                                else if($validate[$post_keys[$i]]["type"] == "checkbox") {
                                    if(isset($post[$post_keys[$i]])) $sql .= "1";
                                    else $sql .= "0";
                                }
                ...
                if(!isset($post["date"]) && isset($validate["url"])) $sql .= "date = now(), ";    
