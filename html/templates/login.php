<?php
    if(isset($_SESSION["user_id"])) header("location:/happycms/");
?>
...
<head>
    ...
    <style>
        ...
        .recovery-form { display: none; padding: 10px; background: #f9f9f9; margin-bottom: 10px; border-radius:5px; border:1px solid #f3f3f3; }
    </style>    
</head>
<body>
    <div class="content">
        ...
        <div class="alert-message-info alert alert-block alert-info fade in" style="display: none;"></div>
        ...
        <div class="login">
            ...
            <form action="/happycms/login/processing" method="post" id="login">
                ...
                <a href="" onclick="password_remember(); return false;">Oh no! I don’t remember my password</a>
                <div class="recovery-form" id="recovery-form"></div>
    ...
    <script>
        ...
        $(document).ready(function(){
           ...
           $('#login').validate({
            ...
            $("#user-email").keyup(function(k){
                if(k.keyCode == 13) recovery();
            });
        });
        ...
        function password_remember(){
            if($("#recovery-form").css("display") == "none") {
                $("#recovery-form").html('<label>Which is your username or email? </label><input type="text" id="user-email" placeholder="Give me your email or username"/><a href="" onclick="recovery(); return false;">Send</a>').fadeIn(400);
                $("#user-email").focus();
            } else {
                $("#recovery-form").fadeOut(400,function(){ $("#recovery-form").html(''); })
            }
        ...
        function recovery(){
            ...
            if(value != ""){
                ...
                $.ajax({
                    ...
                    type:"post",
                    cache:false,
                    data:{val:value,fld:field},
                    beforeSend:function(){
                        $("#recovery-form a").css("color","#ccc").attr("onclick","");
                    },
                    success:function(data){
                        $("#recovery-form a").css("color","#0199ca").attr("onclick","recovery(); return false;");
                        if(data==1){
                            $("#recovery-form").fadeOut(400);
                            $("#user-email").val("");
                            $(".alert-message-info").html("The new password was sended to your email, check it and restore it the fun.").fadeIn(200);
                        }
                    }
                });    
            }            
